...
The following table shows the types of monitoring locations, the 
```{r echo=TRUE}
...
#TODO maybe for each of the parameters of interest we pull in lat/long and plot the locations of stations. Alternatively,
use the current map plot and refine by being able to differentiate what is being monitored at each of the points
...
```{r}
# get lat/longs
do_url_call <- paste0('https://www.waterqualitydata.us/Result/search?huc=', huc_code, '&mimeType=tsv&sorted=no')

do_url_station <- gsub("/Result/", "/Station/", do_url_call)

do_site_info <- importWQP(do_url_station, FALSE, tz="") |>
  janitor::clean_names() |> glimpse()
  # select(monitoring_location_identifier, huc_eight_digit_code, latitude_measure, longitude_measure)

#TODO the idea here would be to join to actual data and summarize so that we can see where the stations that collect DO are located. Code below is here just for reference, probably need to call for readWQPdata function
# join 

do_data_info <- readWQPdata(huc = huc_code,
    characteristicName = do,
    startDateLo = "01-01-2020",
    startDateHi = "01-01-2021") |> 
  clean_names()
    
    
all_do_data <- do_data_info |> left_join(do_site_info) |> 
...
color_palette <- colorFactor(
  palette = "Set1", # You can use "Set1", "Set2", "Dark2", or any RColorBrewer palette
  domain = all_do_data$organization_formal_name
)
...
leaflet(data = all_do_data)  |> 
  ...
  addCircleMarkers(
    ...
    popup = ~paste0("<br><b>Organization Name: </b>",organization_formal_name ,
...
Overview of parameters available (`CharacteristicName`) for Klamath Basin HUC code
...
# ---- here I am trying to get an overview of what parameters (characteristicName) are available, will pull 1 year's data as a sample. There are too many so we should figure out which ones we'd like to highlight other than turbidity, pH, temp and DO
