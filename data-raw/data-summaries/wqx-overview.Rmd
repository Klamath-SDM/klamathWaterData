...
Glimpse of turbidity data pull 

```{r echo=FALSE}
tb_data <- readWQPdata(huc = huc_code,                       
...
```{r include=FALSE}
#check that it is only temp data 
unique(tb_data$characteristic_name)
```


```{r include=FALSE}
# join 
all_tb_data <- tb_data |> left_join(site_info) |> 
  mutate(latitude_measure = as.numeric(latitude_measure),
         longitude_measure = as.numeric(longitude_measure)) |> 
  glimpse()
```
...
```{r echo=FALSE}
# Count number of records per date
daily_tb_record_counts <- all_tb_data |> 
  group_by(activity_start_date) |> 
  summarise(record_count = n(), .groups = "drop") |> 
  mutate(year = year(activity_start_date),
         month = month(activity_start_date, label = TRUE),
...
daily_tb_record_counts_table <- daily_tb_record_counts |> 
  select(activity_start_date, record_count)

DT::datatable(daily_tb_record_counts_table, options = list(pageLength = 10))
```

Plot below shows the same data than table. This indicates that data is not being collected every day

```{r echo=FALSE, warning=FALSE}
ggplot(daily_tb_record_counts, aes(x = day, y = record_count)) +
  geom_col(fill = "steelblue") +
  facet_wrap(~month) +  
  scale_x_continuous(breaks = seq(1, 30, by = 5), limits = c(1, 30)) +  
  labs(title = "Number of Turbidity Records per Day (2020 - 2024)",
       x = "Day of the Month",
       y = "Record Count") +
  theme_minimal()
```
...
```{r echo=FALSE}
monthly_tb_record_counts_persite <- all_tb_data |>
  mutate(
    year = year(activity_start_date),
    month = month(activity_start_date, label = TRUE)
  ) |>
...
monthly_tb_record_counts_persite_table <- monthly_tb_record_counts_persite |> 
  select(monitoring_location_identifier, record_count, year, month)

DT::datatable(monthly_tb_record_counts_persite_table, options = list(pageLength = 10))
```
...
```{r echo=FALSE}
# Table below shows the amount of data entries per site, per month
yearly_tb_record_counts_persite <- all_tb_data |>
  mutate(
    year = year(activity_start_date),
    month = month(activity_start_date, label = TRUE)
  ) |>
...
yearly_tb_record_counts_persite_table <- yearly_tb_record_counts_persite |> 
  select(monitoring_location_identifier, record_count, year)

DT::datatable(yearly_tb_record_counts_persite_table, options = list(pageLength = 10))
```
...
```{r echo=FALSE}
# Extract year for faceting
all_tb_data <- all_tb_data |> mutate(year = year(activity_start_date))


yearly_summary_tb <- all_tb_data |> 
    group_by(organization_identifier) |> 
    summarize(year = year,
              total_records = n(),
              .groups = "drop")

ggplot(yearly_summary_tb, aes(x = organization_identifier, y = total_records, fill = organization_identifier)) +
  geom_bar(stat = "identity") +
  facet_wrap(~year, scales = "free_y") +  
  theme_minimal() +
  labs(title = "Total Turbidity Records by Organization for Each Year",
       x = "Organization Identifier",
       y = "Total Records") +
...
```{r echo=FALSE, warning=FALSE}
# Ensure date is properly formatted
tubidity_data <- all_tb_data |> mutate(activity_start_date = as.Date(activity_start_date),
                       result_measure_value = as.numeric(result_measure_value))

summary(tubidity_data$result_measure_value)
tubidity_data <- tubidity_data |> filter(!is.na(result_measure_value))

ggplot(tubidity_data, aes(x = activity_start_date, y = result_measure_value)) +
  geom_point(alpha = 0.5, size = 1) + 
  facet_wrap(~organization_formal_name, scales = "free_y") +  
  scale_y_continuous(breaks = pretty_breaks(n = 5)) +  
  labs(title = "Turbidity Over Time by Agency (2020-2024)",
       x = "Date",
       y = "Turbidity (FNU)") +
  theme_minimal() +
  theme(legend.position = "none", 
...
Glimpse of Total Suspended Solids data pull 

```{r echo=FALSE}
tss_data <- readWQPdata(huc = huc_code,                       
...
```{r include=FALSE}
#check that it is only temp data 
unique(tss_data$characteristic_name)
```


```{r include=FALSE}
# join 
all_tss_data <- tss_data |> left_join(site_info) |> 
  mutate(latitude_measure = as.numeric(latitude_measure),
         longitude_measure = as.numeric(longitude_measure)) |> 
  glimpse()
```
...
```{r echo=FALSE}
daily_tss_record_counts <- all_tss_data |> 
  group_by(activity_start_date) |> 
  summarise(record_count = n(), .groups = "drop") |> 
  mutate(year = year(activity_start_date),
         month = month(activity_start_date, label = TRUE),
...
daily_tss_record_counts_table <- daily_tss_record_counts |> 
  select(activity_start_date, record_count)

DT::datatable(daily_tss_record_counts_table, options = list(pageLength = 10))
```

Plot below shows the same data than table. This indicates that data is not being collected every day

```{r echo=FALSE, warning=FALSE}
ggplot(daily_tss_record_counts, aes(x = day, y = record_count)) +
  geom_col(fill = "steelblue") +
  facet_wrap(~month) +  
  scale_x_continuous(breaks = seq(1, 30, by = 5), limits = c(1, 30)) +  
  labs(title = "Number of Total Suspended Solids (TSS) Records per Day (2020 - 2024)",
       x = "Day of the Month",
       y = "Record Count") +
  theme_minimal()
```
...
```{r echo=FALSE}
monthly_tss_record_counts_persite <- all_tss_data |>
  mutate(
    year = year(activity_start_date),
    month = month(activity_start_date, label = TRUE)
  ) |>
...
monthly_tss_record_counts_persite_table <- monthly_tss_record_counts_persite |> 
  select(monitoring_location_identifier, record_count, year, month)

DT::datatable(monthly_tss_record_counts_persite_table, options = list(pageLength = 10))
```
...
```{r echo=FALSE}
# Table below shows the amount of data entries per site, per month
yearly_tss_record_counts_persite <- all_tss_data |>
  mutate(
    year = year(activity_start_date),
    month = month(activity_start_date, label = TRUE)
  ) |>
...
yearly_tss_record_counts_persite_table <- yearly_tss_record_counts_persite |> 
  select(monitoring_location_identifier, record_count, year)

DT::datatable(yearly_tss_record_counts_persite_table, options = list(pageLength = 10))
```
...
```{r echo=FALSE}
# Extract year for faceting
all_tss_data <- all_tss_data |> mutate(year = year(activity_start_date))


yearly_summary_tss <- all_tss_data |> 
    group_by(organization_identifier) |> 
    summarize(year = year,
              total_records = n(),
              .groups = "drop")

ggplot(yearly_summary_tss, aes(x = organization_identifier, y = total_records, fill = organization_identifier)) +
  geom_bar(stat = "identity") +
  facet_wrap(~year, scales = "free_y") +  
  theme_minimal() +
  labs(title = "Total Total Suspended Solids (TSS) by Organization for Each Year",
       x = "Organization Identifier",
       y = "Total Records") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8), 
          legend.text = element_text(size = 8), 
...
```{r echo=FALSE, warning=FALSE}
# Ensure date is properly formatted
tss_data <- all_tss_data |> mutate(activity_start_date = as.Date(activity_start_date),
                       result_measure_value = as.numeric(result_measure_value))

summary(tss_data$result_measure_value)
tss_data <- tss_data |> filter(!is.na(result_measure_value))

ggplot(tss_data, aes(x = activity_start_date, y = result_measure_value)) +
  geom_point(alpha = 0.5, size = 1) + 
  facet_wrap(~organization_formal_name, scales = "free_y") +  
  scale_y_continuous(breaks = pretty_breaks(n = 5)) +  
  labs(title = "Total Suspended Solids Over Time by Agency (2020-2024)",
       x = "Date",
       y = "TSS (mg/L)") +
  theme_minimal() +
  theme(legend.position = "none", 
