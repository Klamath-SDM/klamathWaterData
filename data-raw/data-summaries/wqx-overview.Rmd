...
These are the water quality parameters available (pulling 2024 data to view parameters). Parameters of interest will be explored on separate tabs 

```{r echo=FALSE}
# Get metadata for all water quality data available for the specified HUC
wqp_metadata <- readWQPdata(huc = huc_code, startDateLo = "2024-01-01",
...
storet_data <- readWQPdata(huc = huc_code,                       
  characteristicName = "Temperature, water",
  startDateLo = "2024-01-01",               
  startDateHi = Sys.Date()) |> 
  janitor::clean_names() |> 
...
unique(storet_data$characteristic_name)
...
# join 
all_storet_data <- storet_data |> left_join(site_info) |> 
...
Looking how many data entries there are for eaech day in 2024 

```{r echo=FALSE}
# Count number of records per date
daily_record_counts <- all_storet_data |> 
...
```{r echo=FALSE}
monthly_record_counts_persite <- all_storet_data |> 
  mutate(
